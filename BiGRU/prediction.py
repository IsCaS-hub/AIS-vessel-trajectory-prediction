...
# Plot predictions and save arrays as CSV files
def plot_prediction(prediction, output_folder='prediction_plot'):
    ...
    input_csv_path = os.path.join(output_folder, f'input_trajectory_{file_name}')
    pd.DataFrame({'lat': input_lat, 'lon': input_lon}).to_csv(input_csv_path, index=False)
    
    destination_csv_path = os.path.join(output_folder, f'destination_groundtruth_{file_name}')
    pd.DataFrame({'lat': destination_lat, 'lon': destination_lon}).to_csv(destination_csv_path, index=False)
    
    future_pred_csv_path = os.path.join(output_folder, f'future_trajectory_pred_{file_name}')
    pd.DataFrame({'lat': future_pred_lat, 'lon': future_pred_lon}).to_csv(future_pred_csv_path, index=False)
